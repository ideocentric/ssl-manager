...
variable "instance_type" {
  description = <<-EOT
    ...
    Note: The CA module generates RSA-4096 keys on demand. t3.small and t4g.small
    complete this in ~1-2 seconds. t3.micro/t4g.micro are slower (~3-5 seconds)
    but remain within the 120-second gunicorn worker timeout.
  EOT
  type        = string
  default     = "t3.small"
}
...
variable "root_volume_size_gb" {
  description = "Root EBS volume size in GB. 30 GB is sufficient for <1000 certificates and CAs + 7-day backups."
  type        = number
  default     = 30
}
