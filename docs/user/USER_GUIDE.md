<!--
  Screenshots in this guide are generated by docs/take_screenshots.py.
  Run that script (not manual screenshots) so the viewport is sized correctly
  before each capture — this prevents the fixed footer from overlapping content.
  See the script's docstring for the full procedure and how to add new shots.
-->
...
![New chain form](screenshots/05-chains/chain_new_form.png)
...
![Search filtering to show only active certificates](screenshots/08-browsing/cert_list_search_active.png)

![Search filtering to show only expired certificates](screenshots/08-browsing/cert_list_search_expired.png)
