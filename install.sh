...
#
# Usage:
#   sudo bash install.sh               # install, or auto-upgrade if already installed
#   sudo bash install.sh --upgrade     # full refresh, preserving config/secret/database
#   sudo bash install.sh --reinstall   # force the interactive installer on an existing install
...
do_upgrade() {
    ...
    # Re-derive existing settings so the regenerated unit/nginx config keep
    # working exactly as before — no re-prompting required.
    local port workers
    port="$(detect_port)"
    ...
    # refreshes app files, dependencies, the systemd unit, and the nginx config
    # while leaving the operator's configuration and data untouched.
    create_user
    create_directories
    copy_app_files
    create_venv
...
copy_app_files() {
    ...
    cp "${SCRIPT_DIR}/remediate_chain_entities.py" "${APP_DIR}/"
...
if [[ "$MODE" == "install" && "${FORCE_FRESH}" -eq 0 ]] && existing_install; then
    ...
    warn "(To force the interactive installer instead, run: sudo bash install.sh --reinstall)"
    do_upgrade
fi
...
warn "  (re-running install.sh on an existing install auto-detects and upgrades)"
